document.addEventListener('DOMContentLoaded', function () {
        ...
        fetch('../Controller/generatorController.php', {
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    setTimeout(() => {
                        messageChannel(data.clientName, data.pdfFile, data.excelFile);
                    }, 2000);
                } else {
    ...
    function messageChannel(name, PDF, Excel) {
        const nombreCliente = name;
        const linkPDF = PDF;
        const linkExcel = Excel;

        const mensaje = `📋 Presupuesto listo\n\n👤 Cliente: *${nombreCliente}*\n🗓️ Fecha: ${new Date().toLocaleDateString()}\n\n📄 PDF: ${linkPDF}\n📊 Excel: ${linkExcel}`;
        const telefonoDestino = '+541153424345';
